...
export function orderEmailTemplate(input: { appUrl: string; firstName?: string | null; status: string; order: Order; transactionReference?: string }) {
  ...
  const rows = order.items.map(item => `<tr><td style="padding:16px 0;border-bottom:1px solid #E2DACC;font-size:14px"><strong>${e(item.name)}</strong><br><span style="font-size:12px;color:#7A7264;line-height:22px">Quantity: ${item.quantity} &middot; ${e(money(item.unitPrice))} each</span></td><td align="right" style="padding:16px 0;border-bottom:1px solid #E2DACC;font-size:14px;white-space:nowrap">${e(money(Number(item.unitPrice) * item.quantity))}</td></tr>`).join('');
  ...
  return { subject: `${title} — SAGI`, ...storeEmail({ ...input, title, preview: intro, text, action: { label: delivered ? 'Leave a review' : 'View your order history', url: url.href },
    body: emailParagraph(intro) + `<h2 style="margin:28px 0 0;font:normal 22px Georgia,serif">Your provisions</h2><table width="100%" cellspacing="0" cellpadding="0" aria-label="Order items">${rows}</table><table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top:16px">${totals.map(([label,value],index) => `<tr><td style="padding:8px 0;font-size:14px;${index===2?'font-weight:bold;border-top:1px solid #E2DACC':''}">${label}</td><td align="right" style="padding:8px 0;font-size:14px;${index===2?'font-weight:bold;border-top:1px solid #E2DACC':''}">${e(money(value))}</td></tr>`).join('')}</table>` + (shipping ? emailParagraph(`Shipping service: ${shipping}`) : '') + (input.transactionReference ? emailParagraph(`Payment reference: ${input.transactionReference}`) : ''),
  }) };
}
