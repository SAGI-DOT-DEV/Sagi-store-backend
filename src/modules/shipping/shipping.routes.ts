...
shippingRouter.post('/rates',authenticate,validate(requestSchema),async(req,res,next)=>{try{
 ...
 if(free){res.json({success:true,data:{freeShippingEligible:true,rates:[{id:'free-shipping',carrier:'SAGI',service:'Free Shipping',amount:'0.00',currency:'CAD'}]}});return;}
 ...
 res.json({success:true,data:{freeShippingEligible:false,shipmentId:shipment.object_id,bestRate,rates}});
