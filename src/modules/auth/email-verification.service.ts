...
export async function sendUserVerificationEmail(input:{email:string;firstName:string;token:string}){
 ...
 if(!env.GMAIL_USER||!env.GMAIL_APP_PASSWORD){await prisma.emailLog.update({where:{id:emailLog.id},data:{status:'FAILED',error:'Gmail is not configured'}});logger.warn({recipient:input.email},'verification email not sent: Gmail is not configured');return false;}
 try{const transport=nodemailer.createTransport({service:'gmail',auth:{user:env.GMAIL_USER,pass:env.GMAIL_APP_PASSWORD.replaceAll(' ','')}});const sent=await transport.sendMail({from:env.EMAIL_FROM,to:input.email,subject:'Confirm your SAGI email address',text:`Hello ${input.firstName}, confirm your email address: ${url.toString()}`,html:`<p>Hello ${input.firstName},</p><p><a href="${url.toString()}">Confirm your email address</a></p><p>This link expires in 24 hours.</p>`});await prisma.emailLog.update({where:{id:emailLog.id},data:{status:'SENT',providerMessageId:sent.messageId,sentAt:new Date()}});return true;}catch(error){await prisma.emailLog.update({where:{id:emailLog.id},data:{status:'FAILED',error:error instanceof Error?error.message.slice(0,1000):'Unknown Gmail error'}});logger.error({err:error,recipient:input.email},'verification email failed');return false;}
}
...
export async function sendExperienceReviewEmail(input:{orderId:string;userId:string;email:string;firstName?:string|null}){
 ...
 if(!env.GMAIL_USER||!env.GMAIL_APP_PASSWORD){const error='Gmail is not configured';await prisma.$transaction([prisma.emailLog.update({where:{id:emailLog.id},data:{status:'FAILED',error}}),prisma.notification.update({where:{id:notification.id},data:{status:'FAILED'}})]);logger.warn({orderId:input.orderId,recipient:input.email},'experience review email not sent: Gmail is not configured');return false;}
 try{const transport=nodemailer.createTransport({service:'gmail',auth:{user:env.GMAIL_USER,pass:env.GMAIL_APP_PASSWORD.replaceAll(' ','')}});const greeting=input.firstName?`Hello ${input.firstName}`:'Hello';const sent=await transport.sendMail({from:env.EMAIL_FROM,to:input.email,subject:'How was your SAGI experience?',text:`${greeting}, your order has been delivered. Tell us about your experience: ${url.toString()}`,html:`<p>${greeting},</p><p>Your order has been delivered. We would love to hear about your experience.</p><p><a href="${url.toString()}">Leave a review</a></p>`});await prisma.$transaction([prisma.emailLog.update({where:{id:emailLog.id},data:{status:'SENT',providerMessageId:sent.messageId,sentAt:new Date()}}),prisma.notification.update({where:{id:notification.id},data:{status:'SENT'}})]);return true;}catch(error){await prisma.$transaction([prisma.emailLog.update({where:{id:emailLog.id},data:{status:'FAILED',error:error instanceof Error?error.message.slice(0,1000):'Unknown Gmail error'}}),prisma.notification.update({where:{id:notification.id},data:{status:'FAILED'}})]);logger.error({err:error,orderId:input.orderId,recipient:input.email},'experience review email failed');return false;}
}
...
export async function sendOrderEmail(input:{orderId:string;userId:string;email:string;firstName?:string|null;status:string;transactionReference?:string;amount?:string;currency?:string}){
 ...
 if(!env.GMAIL_USER||!env.GMAIL_APP_PASSWORD){await prisma.emailLog.update({where:{id:log.id},data:{status:'FAILED',error:'Gmail is not configured'}});logger.warn({orderId:input.orderId,recipient:input.email},'order email not sent: Gmail is not configured');return false;}
 try{
  ...
  const transport=nodemailer.createTransport({service:'gmail',auth:{user:env.GMAIL_USER,pass:env.GMAIL_APP_PASSWORD.replaceAll(' ','')}});
  const sent=await transport.sendMail({from:env.EMAIL_FROM,to:input.email,subject,text,html});
