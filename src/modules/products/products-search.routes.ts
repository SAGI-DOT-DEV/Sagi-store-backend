...
const listSchema=z.object({body:z.object({}).default({}),params:z.object({}),query:z.object({q:z.string().trim().max(200).optional(),categoryId:z.string().cuid().optional(),category:z.string().trim().max(100).optional(),minPrice:z.coerce.number().nonnegative().optional(),maxPrice:z.coerce.number().nonnegative().optional(),page:z.coerce.number().int().positive().default(1),limit:z.coerce.number().int().positive().max(100).default(20),sort:z.enum(['createdAt','name','price']).default('createdAt'),order:z.enum(['asc','desc']).default('desc')})});

export const productsSearchRouter=Router();
productsSearchRouter.get('/',validate(listSchema),async(req,res,next)=>{try{
 const q=req.query as unknown as {q?:string;categoryId?:string;category?:string;minPrice?:number;maxPrice?:number;page:number;limit:number;sort:'createdAt'|'name'|'price';order:'asc'|'desc'};
 const where={status:'ACTIVE' as const,...(q.categoryId?{categoryId:q.categoryId}:{}),...(q.category?{category:{slug:q.category}}:{}),...(q.minPrice!==undefined||q.maxPrice!==undefined?{variants:{some:{price:{...(q.minPrice!==undefined?{gte:q.minPrice}:{}),...(q.maxPrice!==undefined?{lte:q.maxPrice}:{})}}}}:{}),...(q.q?{OR:[{name:{contains:q.q,mode:'insensitive' as const}},{description:{contains:q.q,mode:'insensitive' as const}},{variants:{some:{OR:[{sku:{contains:q.q,mode:'insensitive' as const}},{name:{contains:q.q,mode:'insensitive' as const}}]}}}]}:{})};
