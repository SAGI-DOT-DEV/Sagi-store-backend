...
export class OrdersService {
 async create(userId:string,addressId:string,shippingRateId:string){
  ...
  if(shippingRateId && shippingRateId !== 'free-shipping'){
  ...
  return retryWriteConflict(()=>prisma.$transaction(async tx=>{
   ...
   if(shippingRateId === 'free-shipping'){
    const settings=await tx.shippingSettings.findUnique({where:{id:'default'}});
    if(!settings || settings.freeShippingThreshold===null || subtotal.lessThan(settings.freeShippingThreshold))throw new ValidationError('This order is no longer eligible for free shipping');
    shipping={amount:new Prisma.Decimal(0),currency:settings.currency,carrier:'SAGI',service:'Free Shipping'};
   }
   ...
   // Keep cart items until Stripe confirms payment; abandoned checkouts must
   // not empty the customer's cart.
